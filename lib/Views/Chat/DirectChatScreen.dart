            ...
            widget.isBlocked == "1"
                ? Center(
                    child: Text(
                      "Blocked",
                      style: ThemeHelper()
                          .TextStyleMethod1(12, context, FontWeight.w600),
                    ),
                  )
                : Container(),
            Container(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            ...
                            if (todaysDate.difference(date).inSeconds < 60) {
                              ...
                              countTime = todaysDate
                                      ...
                                      .toString() +
                                  " mins ago";
                            } else if (todaysDate.difference(date).inHours <
                                24) {
                              countTime = todaysDate
                                      .difference(date)
                                      .inHours
...
class Items {
  ...
  // $query3 = "Select *  From `Exchange` Where (user_id=$sender_id AND exchange_id=$receiver_id) OR (user_id=$receiver_id AND exchange_id=$sender_id)";
  //                $run=mysqli_query($conn, $query3);
  //                if (mysqli_num_rows($run) == 0){
  //               $query2 = "Insert into `Exchange`( `user_id`, `exchange_id`, `move_to_connection`) VALUES ($sender_id,$receiver_id,0);";
